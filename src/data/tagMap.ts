 ...
 */
export function resolveProvenance(def: TagDefinition, mode: OperatingMode): Provenance {
  if (mode === 'SIMULATION') return def.simulationProvenance
...
export function isUnavailable(tagName: string, mode: OperatingMode): boolean {
 ...
 */
export function evaluateStatus(def: TagDefinition, value: TagValue): TagStatus {
  if (typeof value !== 'number' || !Number.isFinite(value)) return 'UNKNOWN'
  const l = def.limits
  if (!l) return 'NORMAL'

  if (l.tripHigh !== undefined && value >= l.tripHigh) return 'TRIP'
  if (l.alarmHigh !== undefined && value >= l.alarmHigh) return 'ALARM'
  if (l.alarmLow !== undefined && value <= l.alarmLow) return 'ALARM'
  if (l.warningHigh !== undefined && value >= l.warningHigh) return 'WARNING'
  if (l.warningLow !== undefined && value <= l.warningLow) return 'WARNING'
  return 'NORMAL'
}
 ...
 */
function resolveQuality(provenance: Provenance, mode: OperatingMode, stale: boolean): TagQuality {
  if (provenance === 'UNAVAILABLE') return 'NO_TAG'
  if (stale) return 'STALE'
  if (mode === 'LIVE') return 'GOOD'
  return 'SIMULATION'
}

export interface MakeTagOptions {
  timestamp: number
  mode: OperatingMode
  stale?: boolean
}
...
export function makeTag(
  tagName: string,
  value: TagValue,
  { timestamp, mode, stale = false }: MakeTagOptions,
): Tag {
  const def = getTagDefinition(tagName)
...
export function numericValue(tag: Tag | undefined): number | null {
