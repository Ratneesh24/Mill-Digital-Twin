...
const NOTE = {
    NO_FORCE: 'Roll separating force is not in the 6-month CRM04 extract. Estimated from MILL_ACT_TRQ and the reduction model — indication of trend only, not a calibrated force reading. Priority tag for OEM (§22 item 3).',
    NO_LVDT: 'No HAGC/LVDT position tag in the extract. Derived from the gaugemeter inverse S0 = h - F/M, using the ESTIMATED force — so its accuracy is bounded by the force estimate.',
    NO_BENDING: 'No work roll bending force tag in the extract. Displayed as NO TAG; the twin must not animate a fake roll bend (§7.4).',
    NO_WR_RPM: 'No work roll speed tag in the extract. Derived from mill speed and roll diameter (§8.4).',
    NO_OSDS: 'No per-side instrumentation in the extract. OS/DS split and tilt are carried in the data model (§2 BUR taper / -9 t differential issue) but have no value on this feed.',
    NO_COOLANT: 'No coolant / lubrication / exhaust status tags in the extract.',
    NO_MILL_SPEED_ACTUAL: 'No direct mill speed feedback tag. Derived from the rate of change of the reel length tags; at 5 s historian sampling this is a coarse average, not an instantaneous speed.',
    DERIVED_FROM_STATE: 'Not a PLC tag. Derived inside the twin from tags that are present on the feed.',
};
function def(d) {
    return d;
}
 ...
 * namespace.
 */
function buildReelTags(reel, name) {
...
export const tagDefinitionMap = new Map(tagDefinitions.map((d) => [d.tagName, d]));
