...
const eventLog = new BoundedLog(EVENT_LOG_CAPACITY);
let eventSeq = 0;
...
export const useMachineStore = create((set, get) => ({
    ...
    async connect(mode, url) {
        ...
        unsubscribeConn = source.onConnectionChange((info) => {
            ...
            get().addEvent('COMMS', info.status === 'CONNECTED'
        ...
        unsubscribeData = source.subscribe([], ({ timestamp, tags }) => {
            ...
            frameTimes.push(now);
            while (frameTimes.length > 0 && now - frameTimes[0] > 2000)
                frameTimes.shift();
            ...
            const diagnostics = source instanceof SimulationDataSource
        ...
        watchdog = setInterval(() => {
            ...
            if (!prevState.communication.lastFrameTimestamp)
                return;
            ...
            if (becameStale) {
                get().addEvent('COMMS', `DATA STALE — last valid data ${new Date(prevState.communication.lastValidTimestamp).toLocaleTimeString()}`);
            }
        }, 500);
...
function markTagsStale(tags) {
    return Object.fromEntries(Object.entries(tags).map(([name, tag]) => [
        name,
        tag.quality === 'NO_TAG' || tag.quality === 'BAD' ? tag : { ...tag, quality: 'STALE' },
    ]));
}
function interlocksFor(state) {
...
function applyCommunication(set, get, communication) {
    const prev = get();
 ...
 */
function applyFrame(set, get, next, tags) {
    const prev = get().state;
    // ---- Events worth recording on the timeline (§11.4) --------------------
    if (prev.machineStatus !== next.machineStatus) {
        get().addEvent('STATE', `${STATUS_LABEL[next.machineStatus]}${next.statusReason ? ` — ${next.statusReason}` : ''}`);
    }
    if (prev.pass.current !== next.pass.current) {
        get().addEvent('PASS', `PASS ${next.pass.current} STARTED — ${next.pass.inputThickness.toFixed(3)} → ${next.pass.targetThickness.toFixed(3)} mm, ${next.pass.reduction.toFixed(1)}%`);
    }
    ...
    if (Math.abs(prev.speed.reference - next.speed.reference) >= 1) {
        get().addEvent('SETPOINT', `SPEED ${prev.speed.reference.toFixed(0)}→${next.speed.reference.toFixed(0)} mpm`);
    }
...
// ---------------------------------------------------------------------------
export const selectMachineState = (s) => s.state;
export const selectStatus = (s) => s.state.machineStatus;
export const selectDirection = (s) => s.state.rollingDirection;
export const selectMode = (s) => s.state.operatingMode;
export const selectCommunication = (s) => s.state.communication;
export const selectInterlockChain = (s) => s.interlockChain;
...
export function readMachineState() {
