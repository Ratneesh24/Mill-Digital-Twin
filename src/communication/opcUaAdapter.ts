...
export interface OpcUaNodeMapping {
  /** Twin tag name — the key the WebSocket payload must use. */
  tagName: string
  /** Proposed OPC-UA node id on the mill server. */
  nodeId: string
  /** Suggested sampling interval, ms. */
  samplingIntervalMs: number
  /** Suggested deadband (absolute) to keep bandwidth sane. */
  deadband: number
  /** Whether the tag is believed to exist today (§7.4). */
 ...
 */
function samplingFor(tagName: string): number {
  if (
    tagName.startsWith('ROLL.FORCE') ||
...
function deadbandFor(unit: string | undefined): number {
  switch (unit) {
    case 'mm':
      return 0.001
    case 'µm':
...
export const opcUaMapping: OpcUaNodeMapping[] = tagDefinitions.map((def) => ({
  tagName: def.tagName,
...
export function gatewayConfigSketch(): string {
