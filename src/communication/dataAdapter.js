...
function bool(tags, tagName) {
    ...
    if (typeof tag.value === 'string')
        return tag.value === 'true' || tag.value === 'ON';
    return null;
}
function health(tags, tagName) {
    const tag = tags[tagName];
    if (!tag || tag.value === null)
        return 'NO_TAG';
    const v = String(tag.value);
    if (v === 'HEALTHY' || v === 'WARNING' || v === 'FAULT' || v === 'OFF')
        return v;
    return 'UNKNOWN';
}
function ctrl(tags, tagName) {
    const tag = tags[tagName];
    if (!tag || tag.value === null)
        return 'NO_TAG';
    const v = String(tag.value);
    if (v === 'ON' || v === 'OFF' || v === 'FAULT')
        return v;
    return 'UNKNOWN';
}
/** Subtraction that propagates unavailability instead of turning it into 0. */
function diff(a, b, scale = 1) {
    if (a === null || b === null)
        return null;
    return (a - b) * scale;
}
...
// Sub-projections
// ---------------------------------------------------------------------------
function projectReel(tags, reel) {
    const roleRaw = str(tags, `${reel}.ROLE`);
    const role = roleRaw === 'PAYOFF' || roleRaw === 'TENSION' || roleRaw === 'IDLE'
        ? roleRaw
        : reel === 'POR'
            ? 'IDLE'
            : 'IDLE';
    ...
    return {
        ...
        status: statusRaw === 'RUNNING' || statusRaw === 'STOPPED' || statusRaw === 'FAULT'
            ? statusRaw
            : 'UNKNOWN',
    };
}
function projectRoll(tags, rpmTag, bendingTag, actualDiameterTag, nominalDiameter, surfaceSpeed) {
    return {
...
function projectGauge(tags, thicknessTag, readyTag, referenceThickness) {
...
export function projectMachineState(tags, ctx) {
    ...
    return {
        ...
        drive: {
            ...
            torquePercentage: (numOr(tags, 'DRIVE.TORQUE', 0) / millConfig.ratings.mainDriveRatedTorque) * 100,
        },
        hydraulics: {
        ...
        gauges: {
            dtr: projectGauge(tags, 'GAUGE.DTR.THICKNESS', 'GAUGE.DTR.READY', 
            ...
            etr: projectGauge(tags, 'GAUGE.ETR.THICKNESS', 'GAUGE.ETR.READY', rollingDirection === 'FORWARD' ? thicknessReference : null),
        },
        interlocks: {
 ...
 */
export function emptyMachineState(mode) {
    return projectMachineState({}, {
        mode,
        communication: {
            connected: false,
