...
import type { MachineStatus, OperatingMode } from '../types/machine'
import type { TagFrame, TagValue } from '../types/tags'
...
const requiredNumericTags = [
  ...
  ...['DTR', 'ETR', 'POR'].flatMap((reel) =>
    ['TENSION', 'DIAMETER', 'LENGTH', 'TORQUE', 'CURRENT', 'RPM'].map((field) => `${reel}.${field}`),
  ),
...
export interface WebSocketDataSourceOptions {
  url: string
  /** Nominal publish rate advertised by the backend, Hz (§14.2: 10/5/2/1/0.2). */
...
export class WebSocketDataSource extends BaseDataSource {
  ...
  readonly mode: OperatingMode = 'LIVE'
  ...
  constructor({ url, nominalRateHz = 5, maxBackoffMs = 15000 }: WebSocketDataSourceOptions) {
  ...
  private open(): void {
    ...
    const isCurrent = () =>
      !this.closedByUs && generation === this.generation && this.socket === socket
   ...
   */
  private parseFrame(data: unknown): { timestamp: number; tags: TagFrame } | null {
    const reject = (error: string) => {
      this.setConnection({ status: 'ERROR', error })
      return null
    }
    if (typeof data !== 'string') return reject('Gateway frame must be JSON text')
    let payload: unknown
    try {
    ...
    if (timestamp <= this.lastTimestamp) return reject('Replayed or out-of-order gateway frame')

    const tags: TagFrame = {}
    for (const [tagName, raw] of Object.entries(payload.tags)) {
      if (!getTagDefinition(tagName)) continue // unknown tag: not displayed
      if (!validValue(tagName, raw)) return reject(`Invalid gateway value for ${tagName}`)
      tags[tagName] = makeTag(tagName, raw, { timestamp, mode: this.mode })
    }
    if (Object.keys(tags).length === 0) return reject('Gateway frame contained no known tags')
...
function validValue(tagName: string, raw: unknown): raw is TagValue {
  const def = getTagDefinition(tagName)!
  if (raw === null) return def.liveAvailability === 'UNAVAILABLE'
  ...
  if (tagName === 'MILL.STATUS') return (machineStatuses as readonly string[]).includes(raw)
  if (tagName === 'MILL.DIRECTION') return raw === 'FORWARD' || raw === 'REVERSE'
  if (tagName.endsWith('.ROLE')) return ['PAYOFF', 'TENSION', 'IDLE'].includes(raw)
  if (tagName.endsWith('.BRAKE')) return ['APPLIED', 'RELEASED'].includes(raw)
  ...
  if (tagName.endsWith('.STATUS')) return ['ON', 'OFF', 'FAULT', 'UNKNOWN', 'NO_TAG'].includes(raw)
