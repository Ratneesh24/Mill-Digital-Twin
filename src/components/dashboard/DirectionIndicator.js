...
export function DirectionIndicator() {
    ...
    return (_jsxs("div", { className: "border-line bg-base-950 flex items-center gap-2 rounded-full border px-3 py-1.5 shadow-sm", children: [_jsx("span", { className: "label", children: "DIRECTION" }), _jsx("div", { className: "relative flex h-4 w-16 items-center overflow-hidden", children: [0, 1, 2].map((i) => (_jsx(motion.span, { className: `absolute h-[2px] w-3 ${reversing ? 'bg-warning' : 'bg-normal'}`, animate: moving
                        ? { x: forward ? [0, 56] : [56, 0], opacity: [0, 1, 1, 0] }
                        : { x: forward ? 20 + i * 14 : 36 - i * 14, opacity: 0.3 }, transition: moving
                        ? { duration: 1.1, repeat: Infinity, delay: i * 0.36, ease: 'linear' }
                        : { duration: 0.3 } }, i))) }), _jsx("span", { className: `num text-meta font-semibold tracking-[0.1em] ${reversing ? 'text-warning' : 'text-text'}`, children: forward ? 'FORWARD →' : '← REVERSE' }), _jsxs("span", { className: "text-text-faint border-line border-l pl-2 text-micro tracking-wider", children: [entryReelId(direction), " \u2192 ", exitReelId(direction)] })] }));
}
