...
export function ConnectionStatus() {
  ...
  return (
    <div className="flex min-w-0 max-w-full flex-wrap items-center gap-x-3 gap-y-2">
      ...
      >
        {(['SIMULATION', 'SIM_46TAG', 'LIVE'] as OperatingMode[]).map((m) => (
          <button
            ...
            className={`min-h-9 rounded-full px-3.5 py-1.5 text-[11px] font-semibold tracking-[0.06em] transition-colors ${
              mode === m
                ? m === 'LIVE'
      ...
      {mode === 'LIVE' &&
        (editingUrl ? (
          <label className="text-text-dim flex min-w-0 max-w-full flex-wrap items-center gap-2 text-[11px] font-medium">
            ...
            <input
              ...
              className="border-line bg-base-900 text-text num focus:border-normal w-[240px] min-w-0 max-w-full rounded-md border px-2.5 py-2 text-[11px]"
            />
          </label>
        ) : (
          <button
            ...
            className="text-text-faint num hover:text-text hover:bg-base-800 min-w-0 max-w-full rounded-md break-all px-2 py-1.5 text-left text-[11px]"
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="label">{mode === 'LIVE' ? 'PLC CONNECTION' : 'SOURCE'}</span>
        <span
        ...
        >
          <span
            ...
            className={`h-1.5 w-1.5 rounded-full ${
              healthy ? 'bg-healthy' : 'bg-alarm alarm-pulse'
            }`}
          />
      ...
      {!healthy && (
        <span className="text-text-faint num text-[11px]">
          LAST VALID DATA:{' '}
      ...
      {healthy && (
        <span className="text-text-faint num text-[11px]">
          {comm.updateRateHz.toFixed(1)} Hz · {comm.framesReceived} frames
        </span>
      )}
    </div>
  )
