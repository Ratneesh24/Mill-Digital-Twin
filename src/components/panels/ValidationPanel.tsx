...
function withinPct(actual: number, expected: number, tolerancePct: number): boolean {
  if (Math.abs(expected) < 1e-6) return Math.abs(actual) < 1e-3
  return Math.abs((actual - expected) / expected) * 100 <= tolerancePct
}

function invariants(state: MachineState): Check[] {
  const checks: Check[] = []
  ...
  const expectedWrRpm = calculateRollRPM(
    rollSurfaceSpeedFromStripSpeed(state.speed.actual),
    state.rolls.upperWork.diameter,
  )
  checks.push({
  ...
  const expectedBurRpm = calculateRollRPM(
    rollSurfaceSpeedFromStripSpeed(state.speed.actual),
    state.rolls.upperBackup.diameter,
  )
  checks.push({
  ...
  const torqueSignAgrees =
    !rolling || state.rollingForce.actual < 1 || state.drive.torque > 0
  checks.push({
 ...
 */
function provenanceInvariants(tags: TagFrame, state: MachineState): Check[] {
...
function snapshot(state: MachineState): Snapshot {
  return {
...
interface TestDef {
  ...
  expected: string
  run?: () => void
  check: (before: Snapshot, now: MachineState, alarms: string[]) => Check[]
}
...
export function ValidationPanel() {
  ...
  const [baselines, setBaselines] = useState<Record<number, Snapshot>>({})
      ...
      >
        ...
        <div className="border-line mt-3 border-t pt-2">
          ...
          <p className="text-text-faint text-[10px] leading-relaxed">
            ...
            {millConfig.identity.mill}. That requires the §22 open items to close.
          </p>
        </div>
      </Panel>

      <Panel title="Validation tests — §17" bodyClassName="p-0">
        {tests.map((test) => {
                  ...
                  <button
                    ...
                    title={
                      test.run
                        ...
                        : 'Capture a baseline, then wait for the mill to reach this condition'
                    }
                  >
                    {test.run ? 'RUN' : 'ARM'}
                  </button>
...
function ok(label: string, pass: boolean, detail: string): Check {
  return { label, pass, detail }
}

function CheckRow({ check, compact = false }: { check: Check; compact?: boolean }) {
  return (
