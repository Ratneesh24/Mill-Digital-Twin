...
export function FeedDiagnostics() {
  ...
  return (
    <div className="grid min-h-0 gap-3.5 lg:grid-cols-2">
      <Panel title="Data feed">
        <DerivedRow label="Source" value={comm.sourceName} tone="dim" />
        <DerivedRow label="Operating mode" value={state.operatingMode} tone="dim" />
        <DerivedRow
          label="Link"
          value={!comm.connected ? 'LOST' : comm.stale ? 'STALE' : 'CONNECTED'}
          tone={!comm.connected || comm.stale ? 'alarm' : 'healthy'}
        />
        <DerivedRow
          label="Update rate"
        ...
        <DerivedRow
          label="Last valid data"
          value={
            comm.lastValidTimestamp
              ? new Date(comm.lastValidTimestamp).toLocaleTimeString()
              : 'NEVER'
          }
          tone={comm.lastValidTimestamp ? 'dim' : 'alarm'}
        />
      </Panel>

      <Panel title="Physics consistency">
        <DerivedRow
        ...
        <DerivedRow
          ...
          value={hasSolver ? checks.gaugemeterResidualUm.toFixed(3) : 'N/A'}
          unit={hasSolver ? 'µm' : undefined}
          tone={hasSolver ? (checks.gaugemeterOk ? 'healthy' : 'warning') : 'dim'}
          title={
            hasSolver
              ? '|h − (S0 + F/M)| after the coupled solve. Near zero means force, gap and thickness are mutually consistent.'
              : 'No coupled solve runs on a live feed — this residual is only meaningful against the simulation.'
          }
        />
        <DerivedRow
          label="Solver iterations"
          value={hasSolver ? state.diagnostics.solverIterations.toFixed(0) : 'N/A'}
          tone="dim"
          title={
            hasSolver
              ? `Fixed-point iterations used on the last tick, of ${engineeringConfig.solver.maxIterations} allowed`
              : 'No solver runs on a live feed.'
          }
        />
        <DerivedRow
