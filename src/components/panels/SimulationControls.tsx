...
export function SimulationControls() {
  ...
  return (
    <Panel
      ...
      right={
        <span
          className={cn('text-micro tracking-wider', readOnly ? 'text-alarm' : 'text-prov-simulated')}
        >
      ...
      <fieldset disabled={readOnly} className="min-w-0">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          <Cmd
            ...
            onClick={handleStart}
            tone={canStart ? 'healthy' : 'blocked'}
            title={canStart ? 'Start rolling' : (blockReason ?? 'Start rolling')}
          />
          <Cmd label="STOP" icon={<Square size={13} aria-hidden />} onClick={() => send({ type: 'STOP' })} />
          <Cmd label="FAST STOP" icon={<FastForward size={13} aria-hidden />} onClick={() => send({ type: 'FAST_STOP' })} tone="trip" />
          <Cmd
            label="RESET"
            icon={<RotateCcw size={13} aria-hidden />}
            onClick={() => send({ type: 'RESET' })}
            tone={needsReset ? 'reset' : undefined}
            title={needsReset ? 'Clear the latch so the mill can start again' : 'Reset latched stops'}
          />
        </div>
        {!readOnly && blockReason && (
          <p role="status" className="text-warning mt-1.5 flex items-start gap-1 text-micro leading-snug">
        ...
        <div className="border-line mt-2 flex flex-wrap items-center gap-2 border-t pt-2">
          <button
            ...
            className={cn(
              'rounded-[10px] border px-2.5 py-1.5 text-micro tracking-wider transition-colors',
              agc === 'ON'
          ...
          <button
            ...
            className="border-line text-text-dim hover:border-normal/50 hover:text-normal rounded-[10px] border px-2.5 py-1.5 text-micro tracking-wider transition-colors"
          >
            CHARGE NEXT COIL
          </button>
          <span className="text-text-faint num ml-auto text-micro">{status}</span>
        </div>
        ...
        <div className="border-line mt-2 border-t pt-2">
          ...
          <div className="grid grid-cols-2 gap-1">
            {SCENARIOS.map((s) => (
              <button
                ...
                className={cn(
                  'rounded-[10px] border px-2 py-1.5 text-left text-micro leading-tight transition-colors',
                  scenario === s.id
          ...
          <p className="text-text-faint mt-1.5 text-micro leading-snug">
...
function Cmd({
  ...
  onClick: () => void
  tone?: 'healthy' | 'trip' | 'blocked' | 'reset'
  title?: string
}) {
  const toneClass =
    tone === 'healthy'
  ...
  return (
    <button
      ...
      title={title}
      className={cn(
        'inline-flex items-center justify-center gap-1 rounded-[10px] border px-1 py-2 text-micro tracking-wider transition-colors',
        toneClass,
      )}
    >
      {icon}
      ...
      <div className="flex shrink-0 gap-1">
        {steps.map((step) => (
          <button
            ...
            onClick={() => onTrim(step)}
            className="border-line text-text-dim hover:border-normal/50 hover:text-normal num w-11 rounded-md border py-1 text-micro transition-colors"
          >
            {step > 0 ? '+' : ''}
