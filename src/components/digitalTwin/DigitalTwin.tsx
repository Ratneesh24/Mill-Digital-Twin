      ...
      >
        <Suspense fallback={null}>
          <TwinEngineProvider>
            ...
            <SelectableEquipment id="POR"><Coiler reel="POR" /></SelectableEquipment>
            <EquipmentMarker />
            <TensionSystem />
            <ForceVisualization />
            <TwinLabels />
        ...
        <EquipmentInspector />
        <div className="border-line text-text-dim bg-base-900 flex shrink-0 flex-wrap justify-between gap-x-4 gap-y-1 border-t px-3 py-1.5 text-[10px]">
...
function SceneCamera({ zoom }: { zoom: number }) {
  ...
  useEffect(() => {
    ...
    if (!controls) return
    const position = view === 'STAND' ? cameraStand : cameraHome
    const target = view === 'STAND' ? cameraStandTarget : cameraTarget
    ...
    controls.update()
  }, [resetToken, view, aspect, cameraHome, cameraTarget, cameraStand, cameraStandTarget, cameraMinDistance, cameraMaxDistance])
  ...
  return (
    <>
      ...
      <directionalLight
        ...
        shadow-bias={-0.0008}
      />
      <directionalLight position={[-7, 4, -5]} intensity={0.9} color="#d5eaff" />
      {/* Low front fill so the near face of the housing is not a silhouette. */}
      <directionalLight position={[2, 1, 10]} intensity={0.7} color="#edf5fc" />
    </>
  )
}
  ...
  return (
    <div className="stale-hatch pointer-events-none absolute inset-0 z-20 flex items-center justify-center p-3" role="status">
      <div className="border-alarm/70 bg-base-900/95 rounded-lg border px-4 py-3 text-center shadow-sm">
        ...
        <div className="text-text-dim mt-1 text-[11px]">TWIN FROZEN - NOT SHOWING LIVE MACHINE STATE</div>
        <div className="text-text-dim num mt-1 text-[10px]">LAST VALID DATA: {communication.lastValidTimestamp ? new Date(communication.lastValidTimestamp).toLocaleTimeString() : 'NEVER'}</div>
      </div>
    </div>
  )
}
