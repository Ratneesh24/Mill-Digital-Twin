...
const EquipmentContext = createContext({ selected: '', select: () => { } });
export function TwinEquipmentProvider({ children }) {
    const [selected, select] = useState('');
    return _jsx(EquipmentContext.Provider, { value: { selected, select }, children: children });
}
/** Mesh picking and the native selector share view-only state, never process state. */
export function SelectableEquipment({ id, children }) {
    const { select } = useContext(EquipmentContext);
    const gl = useThree((s) => s.gl);
    return (_jsx("group", { onClick: (event) => {
            event.stopPropagation();
            if (event.delta <= 4)
                select(id);
        }, onPointerOver: (event) => { event.stopPropagation(); gl.domElement.style.cursor = 'pointer'; }, onPointerOut: () => { gl.domElement.style.cursor = 'auto'; }, children: children }));
}
export function EquipmentMarker() {
    ...
    return (_jsxs("mesh", { position: [item.x, SCENE.floorY + 0.012, item.z], rotation: [-Math.PI / 2, 0, 0], raycast: () => { }, children: [_jsx("ringGeometry", { args: [outer - 0.07, outer, 64] }), _jsx("meshBasicMaterial", { color: SCENE_COLORS.normal, transparent: true, opacity: 0.8, depthWrite: false })] }));
}
export function EquipmentInspector() {
    ...
    return (_jsxs("section", { "aria-label": "Equipment inspection", className: "border-line bg-base-900 shrink-0 border-t px-3 py-2", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-x-3 gap-y-1.5", children: [_jsx("label", { htmlFor: id, className: "text-text text-xs font-semibold", children: "Inspect equipment" }), _jsxs("select", { id: id, value: selected, onChange: (event) => select(event.target.value), className: "border-line bg-base-950 text-text focus-visible:outline-normal min-h-10 min-w-0 max-w-full rounded-md border px-2 text-xs focus-visible:outline-2 focus-visible:outline-offset-2", children: [_jsx("option", { value: "", children: "Choose equipment" }), Object.entries(EQUIPMENT).map(([key, entry]) => _jsx("option", { value: key, children: entry.name }, key))] }), _jsx("span", { className: "text-text-dim text-meta", children: item ? 'Blue floor ring marks the selected location.' : 'Or click a part in the 3D view.' })] }), item && (_jsxs("div", { className: "mt-2", "aria-label": `${item.name} machine data`, children: [_jsx("p", { className: "text-text-dim mb-2 text-meta", children: item.note }), item.tags ? (_jsx("dl", { className: "grid gap-2", style: { gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 150px), 1fr))' }, children: item.tags.map((tag, index) => (_jsxs("div", { className: "border-line border-l-2 pl-2", children: [_jsx("dt", { className: "text-text-dim text-micro", children: item.labels?.[index] ?? tag }), _jsx("dd", { children: _jsx(ValueReadout, { tagName: tag, size: "sm" }) })] }, tag))) })) : (
                    /*
                    ...
                    */
                    _jsx("p", { className: "border-line text-text-dim border-l-2 pl-2 text-meta", children: "NOT INSTRUMENTED \u2014 no tags for this equipment on the current feed." }))] }))] }));
}
