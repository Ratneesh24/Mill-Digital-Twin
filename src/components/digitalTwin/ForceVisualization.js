...
export function ForceVisualization() {
    ...
    if (!visible)
        return null;
    return (_jsxs("group", { children: [_jsx(ForceArrow, { side: "UPPER", estimated: estimated }), _jsx(ForceArrow, { side: "LOWER", estimated: estimated })] }));
}
function ForceArrow({ side, estimated }) {
    ...
    useTwinFrame((v) => {
        ...
        const color = f >= ALARM_FRACTION ? COLOR_ALARM : f >= WARNING_FRACTION ? COLOR_WARNING : COLOR_NORMAL;
    ...
    return (_jsxs("group", { ref: groupRef, children: [_jsxs("mesh", { ref: shaftRef, children: [_jsx("cylinderGeometry", { args: [SCENE.wrRadius * 0.22, SCENE.wrRadius * 0.22, 1, 12] }), _jsx("meshStandardMaterial", { transparent: true, opacity: 0.75, depthWrite: false })] }), _jsxs("mesh", { ref: headRef, rotation: [Math.PI, 0, 0], children: [_jsx("coneGeometry", { args: [SCENE.wrRadius * 0.48, SCENE.wrRadius * 0.95, 14] }), _jsx("meshStandardMaterial", { transparent: true, opacity: 0.75, depthWrite: false })] })] }));
}
