 ...
 */
function useStripTexture(): CanvasTexture {
  return useMemo(() => {
...
interface SpanProps {
  /** Which half of the pass line this span occupies. */
  sideX: -1 | 1
}
 ...
 */
function StripSpan({ sideX }: SpanProps) {
  ...
  return (
    <group ref={pivotRef}>
      <mesh ref={meshRef} position={[(sideX * spanLength) / 2, 0, 0]} castShadow receiveShadow>
        <boxGeometry args={[spanLength, 1, 1]} />
        <meshStandardMaterial {...MATERIALS.strip} />
      </mesh>
    </group>
  )
}
