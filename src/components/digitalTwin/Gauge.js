...
export function Gauge({ gauge }) {
    ...
    const ready = useMachineStore((s) => gauge === 'ETR' ? s.state.gauges.etr.ready : s.state.gauges.dtr.ready);
    useFrame(() => {
        const head = headRef.current;
        if (!head)
            return;
        const material = head.material;
        ...
        material.color.set(ready === true ? SCENE_COLORS.healthy : ready === false ? SCENE_COLORS.warning : MATERIALS.gaugeHead.color);
    });
    ...
    return (_jsxs("group", { position: [x, 0, 0], children: [_jsxs("mesh", { position: [0, 0, -armDepth / 2 - 0.16], castShadow: true, receiveShadow: true, children: [_jsx("boxGeometry", { args: [0.16, frameHeight, 0.18] }), _jsx("meshStandardMaterial", { ...MATERIALS.gaugeFrame })] }), [-1, 1].map((s) => (_jsxs("mesh", { position: [0, s * frameHeight * 0.4, -0.08], castShadow: true, children: [_jsx("boxGeometry", { args: [0.14, 0.12, armDepth * 0.55] }), _jsx("meshStandardMaterial", { ...MATERIALS.gaugeFrame })] }, s))), _jsxs("mesh", { ref: headRef, position: [0, frameHeight * 0.26, 0], castShadow: true, children: [_jsx("boxGeometry", { args: [0.12, 0.1, 0.14] }), _jsx("meshStandardMaterial", { ...MATERIALS.gaugeHead, emissive: SCENE_COLORS.healthy, emissiveIntensity: 0 })] }), _jsxs("mesh", { position: [0, -frameHeight * 0.26, 0], castShadow: true, children: [_jsx("boxGeometry", { args: [0.12, 0.1, 0.14] }), _jsx("meshStandardMaterial", { ...MATERIALS.gaugeHead })] })] }));
}
