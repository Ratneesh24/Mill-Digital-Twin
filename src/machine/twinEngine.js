...
function normaliseBending(bendingForce) {
    if (bendingForce === null)
        return null;
    return clamp(bendingForce / MAX_BENDING_KN, 0, 1);
}
export function deriveTargets(state) {
    ...
    const payoff = payoffReel(state.rollingDirection) === 'DTR' ? state.tension.dtr : state.tension.etr;
    const winder = tensionReel(state.rollingDirection) === 'DTR' ? state.tension.dtr : state.tension.etr;
    ...
    return {
        ...
        // recomputation here.
        entryStripSpeed: state.thickness.entry > 0
        ...
        entryTensionNormalised: clamp(state.tension.entry / engineeringConfig.tensionLimits.entryMax, 0, 1),
