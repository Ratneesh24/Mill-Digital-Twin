...
export const MOVING_STATES = new Set([
    'THREADING',
    'ROLLING',
    'DECELERATING',
    'SKIN_PASS',
 ...
 */
export function isStartable(status) {
 ...
 */
export function isRolling(status) {
...
export function transition(current, event, ctx) {
    ...
    if (current === 'FAST_STOP') {
        if (event === 'SPEED_ZERO')
            return result('STOPPED', 'Fast stop complete — mill at standstill');
    ...
    if (event === 'INTERLOCK_OK') {
        if (current === 'IDLE')
            return result('READY', 'Mill ready');
    ...
    switch (event) {
        ...
        case 'THREADED':
            if (current === 'THREADING')
                return result('ROLLING', 'Rolling');
            return result(current, 'Threaded');
        case 'AT_SPEED':
            if (current === 'ROLLING')
                return result(current, 'Rolling at speed reference');
            return result(current, 'At speed');
        case 'STOP':
            if (isMoving(current))
                return result('DECELERATING', 'Stop requested — decelerating');
            if (current === 'REVERSING')
                return result('DECELERATING', 'Stop requested during reversal');
 ...
 */
export function beginReversal(current) {
    if (current !== 'STOPPED') {
