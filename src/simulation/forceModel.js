...
import { calculateContactLength, calculateFlattenedRadius, calculateMeanFlowStress, specificTension, } from './rollingModel';
const ZERO_RESULT = {
    forceTonnes: 0,
    forceKN: 0,
    contactLength: 0,
 ...
 */
export function calculateRollingForce(input) {
    const { inputThickness: h0, outputThickness: h1, width: w, workRollDiameter, entryTension, exitTension, accumulatedStrain, flowStressScale = 1, } = input;
    const draft = h0 - h1;
    if (draft <= 1e-6 || w <= 0 || h1 <= 0)
        return ZERO_RESULT;
 ...
 */
export function estimateForceFromTorque(rollTorqueKNm, contactLengthMm) {
