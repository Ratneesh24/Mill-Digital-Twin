...
import { calculateRollingForce } from './forceModel';
/**
 ...
 * only.
 */
export function solveGaugemeter(input) {
 ...
 * MEASURED.
 */
export function inverseGaugemeter(deliveredThicknessMm, forceTonnes) {
 ...
 */
export function gapPositionForTargetThickness(targetThickness, inputThickness, width, workRollDiameter, entryTension, exitTension, accumulatedStrain) {
 ...
 */
export function applyGaugeNoise(trueThicknessMm, rng) {
    const sigmaMm = engineeringConfig.gaugeNoise.sigma / 1000;
    return trueThicknessMm + rng.normal() * sigmaMm;
}
export function hagcStep(state, gapPosition, measuredThickness, referenceThickness, dt, enabled) {
    if (!enabled || dt <= 0)
        return { gapPosition, state };
