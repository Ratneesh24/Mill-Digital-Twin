...
} from '../machine/machineStateMachine'
...
import {
  ...
  tensionReel,
  type ReversalState,
} from '../machine/reversingEngine'
import type { MachineStatus, RollingDirection } from '../types/machine'
import type { PassSchedule } from '../types/coil'
...
import { applyScenario, type ScenarioId, type ScenarioModifiers } from './simulationScenarios'

export type RawFrame = Record<string, TagValue>
...
export interface SimulationCommand {
  ...
  flag?: boolean
  scenario?: ScenarioId
  system?: 'lubrication' | 'coolant' | 'exhaust' | 'lpSystem' | 'hpLoading' | 'hpBending'
}
...
interface SimState {
  ...
  entryTension: number
  exitTension: number
  entryTensionTrim: number
  exitTensionTrim: number

  passNumber: number
  /** Nominal input thickness for the current pass, mm. */
  passInputThickness: number

  /** Length still to be paid off this pass, m. */
  payoffLength: number
  /** Length wound onto the tension reel this pass, m. */
  woundLength: number
  /** Total strip length of the coil at the current input thickness, m. */
  passTotalLength: number
  /** Distance rolled this pass, m — position along the coil for the entry profile. */
  positionM: number

  /** Length on the payoff reel at the START of the pass, m. */
  threaded: boolean
  reversal: ReversalState

  /** POR holds the next coil waiting to be charged. */
  porLength: number
  porThickness: number

  scenario: ScenarioId
  eStop: boolean
  fastStopLatched: boolean
  auxHealth: Record<
    'lubrication' | 'coolant' | 'exhaust' | 'lpSystem' | 'hpLoading' | 'hpBending',
    boolean
  >
  commsHealthy: boolean

  /** Last solved values, retained so a stopped mill reports a coherent frame. */
  deliveredThickness: number
  force: number
 ...
 */
function entryThicknessDeviation(positionM: number, passNumber: number): number {
  const amplitude =
    ENTRY_PROFILE_AMPLITUDE_MM * Math.pow(ENTRY_PROFILE_ATTENUATION, passNumber - 1)
  const p = positionM
  return (
    amplitude *
...
export class SimulationEngine {
  ...
  private readonly schedule: PassSchedule
  private readonly rng = new SeededRandom()
  private pendingEvents: MachineEventType[] = []

  constructor(schedule: PassSchedule = demoPassSchedule) {
  ...
  private buildInitialState(): SimState {
    ...
    const gapPosition = gapPositionForTargetThickness(
      pass.outputThickness,
      pass.inputThickness,
      coil.width,
      millConfig.geometry.workRollDiameter,
      tensions.entryKN,
      tensions.exitKN,
      0,
    )
  ...
  // -------------------------------------------------------------------------
  command(cmd: SimulationCommand): void {
    const s = this.state
    switch (cmd.type) {
      case 'START':
      ...
      case 'SET_SCENARIO':
        ...
        if (s.scenario === 'COMMUNICATION_LOSS') s.commsHealthy = false
        else if (s.scenario !== 'NORMAL') s.commsHealthy = true
        else s.commsHealthy = true
        break
      case 'SET_AUX_HEALTH':
        if (cmd.system) s.auxHealth[cmd.system] = cmd.flag ?? true
        break
    }
  }
  ...
  // The tick.
  // -------------------------------------------------------------------------
  tick(dtSeconds: number): RawFrame {
    const s = this.state
    const dt = clamp(dtSeconds, 0, 0.25) // guard against tab-restore time jumps
    s.timeS += dt

    const coil = this.schedule.coil
    const mods: ScenarioModifiers = applyScenario(s.scenario)
    const wrDiameter = millConfig.geometry.workRollDiameter
    const mandrelRadius = millConfig.geometry.mandrelDiameter / 2

    // ---- 1. Interlocks --------------------------------------------------
    const hydraulicReady =
      s.auxHealth.hpLoading && s.auxHealth.lpSystem && mods.hydraulicPressureFactor > 0.5
    ...
    const entryThickness = Math.max(
      0.05,
      s.passInputThickness + entryThicknessDeviation(s.positionM, s.passNumber),
    )

    const tensionRefs = calculateTensionReferences(
      pass.entrySpecificTension,
      pass.exitSpecificTension,
      entryThickness,
      pass.outputThickness,
    ...
    const auxStatus = (ok: boolean): string => (ok ? 'HEALTHY' : 'FAULT')
    const ctrl = (on: boolean): string => (on ? 'ON' : 'OFF')

    const remainingLength = s.payoffLength
    const passProgress =
      s.passTotalLength > 0 ? clamp(1 - s.payoffLength / s.passTotalLength, 0, 1) * 100 : 0
    ...
    const frame: RawFrame = {
      ...
      'MILL.MASSFLOW.ERROR': massFlowError(
        entryThickness,
        entrySpeed,
        deliveredThickness,
        exitSpeed,
    ...
    writeReel(frame, payoff, {
      ...
      current: reelCurrent(
        payoffTorque,
        millConfig.ratings.reelRatedTorque,
        millConfig.ratings.reelRatedCurrent,
      ),
  ...
  getDiagnostics(): { solverIterations: number; gaugemeterResidualUm: number } {
    return {
   ...
   */
  private stepPassAndReversal(dt: number): void {
    const s = this.state
    ...
    if (s.reversal.phase === 'NONE') return
  ...
  private applyTransition(event: MachineEventType, ctx: TransitionContext): void {
    const s = this.state
  ...
  private applyFlip(): void {
    ...
    s.gapPosition = gapPositionForTargetThickness(
      nextPass.outputThickness,
      nextPass.inputThickness,
      coil.width,
      millConfig.geometry.workRollDiameter,
      tensions.entryKN,
      tensions.exitKN,
      accumulatedStrainBeforePass(nextPassNumber),
    )
    s.gapTrim = 0
  }

  private targetSpeedFor(status: MachineStatus, speedReference: number): number {
    switch (status) {
      case 'ROLLING':
      case 'SKIN_PASS':
...
function directionSignOf(direction: RollingDirection): 1 | -1 {
  return direction === 'FORWARD' ? 1 : -1
}

function rampTowards(current: number, target: number, maxStep: number): number {
  if (current < target) return Math.min(current + maxStep, target)
  if (current > target) return Math.max(current - maxStep, target)
  return current
}

interface ReelFrameValues {
  tension: number
  diameter: number
  length: number
  torque: number
...
function writeReel(frame: RawFrame, reel: 'DTR' | 'ETR', v: ReelFrameValues): void {
...
function coilLengthAtThickness(
  outerRadiusMm: number,
  mandrelRadiusMm: number,
  thicknessMm: number,
): number {
