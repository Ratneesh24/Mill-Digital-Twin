 ...
 *   r = (h0 - h1) / h0 × 100
 */
export function calculateReduction(inputThickness: number, outputThickness: number): number {
 ...
 *   h1 = h0 × (1 - r)
 */
export function outputThicknessFromReduction(inputThickness: number, reductionPct: number): number {
 ...
 */
export function calculateTrueStrain(inputThickness: number, outputThickness: number): number {
 ...
 */
export function calculateMeanFlowStress(
  ...
  const antiderivative = (e: number): number =>
    (kf0 * Math.pow(1 + C * e, n + 1)) / (C * (n + 1))
 ...
 */
export function calculateFlattenedRadius(
  nominalRadiusMm: number,
  forcePerWidthNPerMm: number,
  draftMm: number,
): number {
 ...
 *   L = √(R' · Δh)                                        (§8.2)
 */
export function calculateContactLength(flattenedRadiusMm: number, draftMm: number): number {
 ...
 */
export function rollSurfaceSpeedFromStripSpeed(stripExitSpeedMpm: number): number {
 ...
 */
export function calculateRollRPM(surfaceSpeedMpm: number, rollDiameterMm: number): number {
 ...
 */
export function entrySpeedFromMassFlow(
  exitSpeedMpm: number,
  inputThickness: number,
  outputThickness: number,
): number {
 ...
 */
export function massFlowError(
  inputThickness: number,
  entrySpeedMpm: number,
  outputThickness: number,
  exitSpeedMpm: number,
): number {
 ...
 *   σ = T / (h · w)
 */
export function specificTension(tensionKN: number, thicknessMm: number, widthMm: number): number {
 ...
 *   T = σ · h · w
 */
export function tensionForceFromSpecific(
  specificTensionNPerMm2: number,
  thicknessMm: number,
  widthMm: number,
): number {
