...
export interface ScenarioModifiers {
  /**
   * Multiplies the material's mean flow stress — harder or softer steel than
   * the schedule assumed. This is the lever that actually raises force when AGC
   * is closed: the loop holds the delivered thickness, so a harder coil is
   * absorbed as more force, not as a thicker strip. (Offsetting the gap does
   * nothing under a working AGC, which is exactly what a real mill does.)
   */
  flowStressScale: number
  /** Added to the commanded roll gap S0, mm. Negative = tighter gap. */
  gapOffsetMm: number
  /** Multiplies the resulting force — used only for the trip-path rehearsal. */
  forceScale: number
  /** AGC allowed to close the loop. */
  agcEnabled: boolean
  /** X-ray gauges ready. */
  gaugeReady: boolean
  /** Multiplies the HAGC loading pressure. */
  hydraulicPressureFactor: number
  /**
   * Work roll bending force, kN. `null` means NO TAG — the twin greys the
   * bending readout out and does not animate a bend (§7.4).
   */
  bendingForceKN: number | null
  /** Work roll bending pressure, bar, or null when no tag exists. */
  bendingPressureBar: number | null
}

const BASE: ScenarioModifiers = {
  flowStressScale: 1,
  gapOffsetMm: 0,
  forceScale: 1,
  agcEnabled: true,
...
export interface ScenarioDescriptor {
  ...
  label: string
  description: string
  /** Which §17 validation test this scenario supports. */
  validates?: string
}
...
export function applyScenario(id: ScenarioId): ScenarioModifiers {
  switch (id) {
    case 'HIGH_FORCE':
...
export function getScenario(id: ScenarioId): ScenarioDescriptor {
