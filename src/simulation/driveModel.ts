...
export interface DriveResult {
  /** Total mill spindle torque, kNm. */
  torque: number
  /** Torque contributed by deformation work alone, kNm. */
  rollTorque: number
  /** Torque contributed by the strip tension differential, kNm. */
  tensionTorque: number
  /** Shaft power, kW. */
  power: number
  /** Armature current, A. */
  current: number
  ...
  rpm: number
  /** Torque as a percentage of drive rating. */
...
export interface DriveInput {
  ...
  /** Work roll rotational speed, rpm. */
  rollRPM: number
  /** Work roll diameter, mm. */
  workRollDiameter: number
  /** Entry (back) tension force, kN. */
  entryTensionKN: number
  /** Exit (front) tension force, kN. */
  exitTensionKN: number
  /**
   * True only while material is actually flowing through the bite.
   *
   * A stand parked with the capsule loaded still carries a separating force —
   * the rolls are pressing on stationary strip — but it is not doing any
   * deformation work, so there is no rolling torque. Without this gate the twin
   * shows a stopped mill drawing 200 A, which is the kind of quiet nonsense
   * §18 exists to catch.
   */
  rolling: boolean
}
 ...
 */
export function calculateDrive(input: DriveInput): DriveResult {
  const {
    forceTonnes,
    contactLengthMm,
    rollRPM,
 ...
 */
export function hydraulicPressureFromForce(forceTonnes: number): number {
