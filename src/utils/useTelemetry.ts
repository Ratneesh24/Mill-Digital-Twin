...
import type { TelemetryPoint, TelemetrySignal, TrendWindowKey } from '../types/telemetry'

/** One signal, one window, refreshed at the chart repaint rate. */
export function useTrend(signal: TelemetrySignal, window: TrendWindowKey): TelemetryPoint[] {
  const [points, setPoints] = useState<TelemetryPoint[]>(() => telemetryStore.read(signal, window))

  useEffect(() => {
    setPoints(telemetryStore.read(signal, window))
    const id = setInterval(
      () => setPoints(telemetryStore.read(signal, window)),
      engineeringConfig.chartRefreshMs,
    )
...
export function useMergedTrend(
  ...
  useEffect(() => {
    const list = key ? (key.split('|') as TelemetrySignal[]) : []
