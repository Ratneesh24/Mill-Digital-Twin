...
export interface SignalReference {
  value: number
  label: string
  tone: 'target' | 'warning' | 'alarm'
}

const TONE_COLOR: Record<SignalReference['tone'], string> = {
  target: 'var(--color-inactive)',
  warning: 'var(--color-warning)',
  alarm: 'var(--color-alarm)',
}

export function referenceColor(tone: SignalReference['tone']): string {
  return TONE_COLOR[tone]
}

export function limitsFor(signal: TelemetrySignal): SignalReference[] {
  switch (signal) {
    case 'rollingForce':
      return [
